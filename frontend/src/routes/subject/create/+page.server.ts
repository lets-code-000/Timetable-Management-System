...
export const actions: Actions = {
	default: async (event) => {
		...
		if (!faculty_id?.trim()) fieldErrors.faculty_id = 'Faculty is required';
		if (!department_id?.trim()) fieldErrors.department_id = 'Department is required';

		if (Object.keys(fieldErrors).length > 0) {
			return fail(400, { fieldErrors, values: { name, faculty_id, department_id } });
		}
		...
		try {
			...
			if (!res.ok) {
				...
				return fail(res.status, {
					error: errData.detail || 'Failed to create subject',
					values: { name, faculty_id, department_id }
				});
			}
		} catch (err) {
		...
		redirect(
			303,
			'/subject',
			{ type: 'success', message: 'Subject created successfully' },
			event
		);
	}
};
